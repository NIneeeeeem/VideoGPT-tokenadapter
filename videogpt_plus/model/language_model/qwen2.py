...
class VideoQWENConfig(Qwen2Config):
    model_type = "VideoGPT+"


class VideoQWENModel(MetaModel, Qwen2Model):
    config_class = VideoQWENConfig

    def __init__(self, config: Qwen2Config):
        super(VideoQWENModel, self).__init__(config)


class VideoQWENForCausalLM(Qwen2ForCausalLM, VideoGPTPlusMetaForCausalLM):
    config_class = VideoQWENConfig

    def __init__(self, config):
        super(Qwen2ForCausalLM, self).__init__(config)
        self.model = VideoQWENModel(config)
...
AutoConfig.register("VideoGPT+", VideoQWENConfig)
AutoModelForCausalLM.register(VideoQWENConfig, VideoQWENForCausalLM)
